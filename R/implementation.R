...
# removing minus sign might fix some things
...
time_steps <- seq(7, 72, by = 0.1) # starting with 7:00 on travel day
...
# --- Simulation ---
trial <- simulate_trial(agent = agent_start, time_steps = time_steps)
plot(time_steps, trial$sleep_propensity[-nrow(trial)])
plot(time_steps, trial$states[-nrow(trial)])
